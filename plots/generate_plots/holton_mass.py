...
class HoltonMassModel:
    ...
    def integrate_euler_maruyama(self,x0,t_save,stochastic_flag=True,print_interval=None,seed=2946):
...
if __name__ == "__main__":
    ...
    base_dir = "/home/fabio/work/HM_and_AI_models/Holton_Mass_Model/holton_mass_essentials/"
    ...
    dt_save = 1
    tmax = 4000
    t_stoch = np.arange(0, tmax, dt_save)
    x_stoch = model.integrate_euler_maruyama(x_ffp[-1,:,:], t_stoch)
    ...
    ax.legend()
    fig.savefig(join(savedir,"stochastic_trajectory.png"), **plt_save_kwargs)
    plt.close(fig)

    print(f'{x_stoch.shape = }')
    np.save(join(savedir, "x_stoch.npy"), x_stoch)
