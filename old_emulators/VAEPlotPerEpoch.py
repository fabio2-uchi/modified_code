...
print (f"Input Data shape encoder: {input_data.shape}")
encoder = Convolution1D(64, 2, activation='relu', padding='same')(input_data)
encoder = MaxPooling1D(2)(encoder)  # Downsample: (75 -> 38)
encoder = Convolution1D(64, 2, activation='relu', padding='same')(encoder)
...
cond = encoder
print(cond.shape)
latent_encoding_conditioned = concatenate([latent_encoding, input_data], axis = 1)
print(latent_encoding_conditioned.shape)

# Decoder
decoder_input = Input(shape=(latent_dim + 75,))  # Latent dimensions + auxiliary input
decoder = Dense(576, activation='relu')(decoder_input)  # Fully connected layer to expand dimensions

# Transposed Convolutional Layers + Upsampling
decoder = Conv1DTranspose(64, 2, activation='relu', padding='same')(decoder)
...
decoder_output = Conv1DTranspose(1, 3, activation='linear', padding='same')(decoder)  # Final output shape (75, 1)
...
autoencoder.compile(optimizer='adam', loss=lambda y_true, y_pred: y_pred, run_eagerly=True)
